...
def combine_responses(meter_datas):
    result = meter_datas[0]
    for data in meter_datas[1:]:
        result['total_power'] += data['total_power']
        for i, emeter in enumerate(result['emeters']):
            dmeter = data['emeters'][i]
            emeter['power'] += dmeter['power']
            emeter['current'] += dmeter['current']
            emeter['voltage'] += dmeter['voltage']
            emeter['total'] += dmeter['total']
            emeter['total_returned'] += dmeter['total_returned']
    return result
...
class DbusShelly3emService:
    ...
    def _getShellySerial(self):
        meter_data = self._getShellyData(self._getShellyStatusUrls()[0])
    ...
    def _getShellyStatusUrls(self):
        config = self.config
        return ["http://%s/status" % h for h in config['ONPREMISE']['Hosts'].split(',')]
    ...
    def _update(self):
        config = self.config
        try:
            # get data from Shelly 3em's
            meter_datas = [self._getShellyData(url) for url in self._getShellyStatusUrls()]
            meter_data = combine_responses(meter_datas)
            try:
            ...
            if self.config['DEFAULT']['Role'] in ['grid', 'genset']:
                self._dbusservice['/Ac/Energy/Reverse'] = ((meter_data['emeters'][0]['total_returned'] / 1000) + (meter_data['emeters'][1]['total_returned'] / 1000) + (
            ...
            self._dbusservice['/Ac/Energy/Forward'] = (
                    (meter_data['emeters'][0]['total'] / 1000) + (meter_data['emeters'][1]['total'] / 1000) + (meter_data['emeters'][2]['total'] / 1000))
